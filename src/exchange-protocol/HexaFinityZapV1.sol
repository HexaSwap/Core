...
import {IHexaFinityPair} from "./interfaces/IHexaFinityPair.sol";
import {IHexaFinityRouter02} from "./interfaces/IHexaFinityRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by HexaFinitySwap teams.
 */
contract HexaFinityZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // HexaFinityRouter interface
    IHexaFinityRouter02 public hexaFinityRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from HexaFinitySwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address HexaFinityRouter
    address private hexaFinityRouterAddress;
     ...
     * @param _hexaFinityRouter: address of the HexaFinityRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WBNBAddress,
        address _hexaFinityRouter,
        uint256 _maxZapReverseRatio
    ) {
        WBNBAddress = _WBNBAddress;
        WBNB = IWETH(_WBNBAddress);
        hexaFinityRouterAddress = _hexaFinityRouter;
        hexaFinityRouter = IHexaFinityRouter02(_hexaFinityRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap BNB in a WBNB pool (e.g. WBNB/token)
     * @param _lpToken: LP token address (e.g. HEXA/BNB)
     * @param _tokenAmountOutMin: minimum token amount (e.g. HEXA) to receive in the intermediary swap (e.g. BNB --> HEXA)
     */
     ...
     * @param _lpToken: LP token address (e.g. HEXA/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. HEXA) in the intermediary swap (e.g. BUSD --> HEXA)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. HEXA/WBNB)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in BNB/WBNB) in the intermediary swap (e.g. HEXA --> BNB)
     */
    function zapOutBNB(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. HEXA/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. HEXA or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. HEXA) in the intermediary swap (e.g. BUSD --> HEXA)
     */
    ...
    {
        address token0 = IHexaFinityPair(_lpToken).token0();
        address token1 = IHexaFinityPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IHexaFinityPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = hexaFinityRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = hexaFinityRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IHexaFinityPair(_lpToken).token0() || _token0ToZap == IHexaFinityPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IHexaFinityPair(_lpToken).token0() || _token1ToZap == IHexaFinityPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IHexaFinityPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IHexaFinityPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IHexaFinityPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = hexaFinityRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = hexaFinityRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IHexaFinityPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "HexaFinityRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "HexaFinityRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        hexaFinityRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), hexaFinityRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(hexaFinityRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = hexaFinityRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = hexaFinityRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = hexaFinityRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = hexaFinityRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = hexaFinityRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
