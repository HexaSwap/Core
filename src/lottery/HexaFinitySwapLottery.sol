...
/** @title HexaFinitySwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract HexaFinitySwapLottery is ReentrancyGuard, IHexaFinitySwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInHexa = 50 ether;
    uint256 public minPriceTicketInHexa = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInHexa;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] hexaPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInHexa;
        uint32 finalNumber;
    }
     ...
     * @param _hexaTokenAddress: address of the HEXA token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _hexaTokenAddress, address _randomGeneratorAddress) {
        hexaToken = IERC20(_hexaTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of HEXA to this contract
        uint256 amountHexaToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInHexa,
            _ticketNumbers.length
        );

        // Transfer hexa tokens to this contract
        hexaToken.safeTransferFrom(address(msg.sender), address(this), amountHexaToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInHexa += amountHexaToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInHexaToTransfer
        uint256 rewardInHexaToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInHexaToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        hexaToken.safeTransfer(msg.sender, rewardInHexaToTransfer);

        emit TicketsClaim(msg.sender, rewardInHexaToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in HEXA per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInHexa) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in HEXA for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].hexaPerBracket[j] =
                ...
                // A. No HEXA to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].hexaPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInHexa - amountToShareToWinners);

        // Transfer HEXA to treasury address
        hexaToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in HEXA token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        hexaToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInHexa += _amount;
     ...
     * @param _priceTicketInHexa: price of a ticket in HEXA
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInHexa >= minPriceTicketInHexa) && (_priceTicketInHexa <= maxPriceTicketInHexa),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInHexa: _priceTicketInHexa,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            hexaPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInHexa: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(hexaToken), "Cannot be HEXA token");
    ...
    /**
     * @notice Set HEXA price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInHexa: minimum price of a ticket in HEXA
     * @param _maxPriceTicketInHexa: maximum price of a ticket in HEXA
     */
    function setMinAndMaxTicketPriceInHexa(uint256 _minPriceTicketInHexa, uint256 _maxPriceTicketInHexa)
        external
        onlyOwner
    {
        require(_minPriceTicketInHexa <= _maxPriceTicketInHexa, "minPrice must be < maxPrice");

        minPriceTicketInHexa = _minPriceTicketInHexa;
        maxPriceTicketInHexa = _maxPriceTicketInHexa;
    }
     ...
     * @param _priceTicket price of a ticket (in HEXA)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].hexaPerBracket[_bracket];
        } else {
            return 0;
        }
    }
